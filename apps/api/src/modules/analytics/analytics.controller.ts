...
export class AnalyticsController {
  ...
  async dashboard(): Promise<unknown> {
    const [activeCampaigns, leadCount, qualifiedCount, metricTotals, aiSpend] = await Promise.all([
      this.db.campaign.count({ where: { status: 'ACTIVE', deletedAt: null } }),
      this.db.lead.count({ where: { deletedAt: null } }),
      this.db.lead.count({ where: { status: 'QUALIFIED', deletedAt: null } }),
      this.db.metricDaily.aggregate({
        _sum: { spend: true, revenue: true, leads: true, conversions: true, clicks: true },
      }),
      this.db.aiUsage.aggregate({ _sum: { costUsd: true } }),
    ])
  ...
  async channels(): Promise<unknown[]> {
    const grouped = await this.db.metricDaily.groupBy({
      by: ['channel'],
