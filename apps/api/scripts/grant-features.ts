...
async function main(): Promise<void> {
  ...
  try {
    ...
    const wanted = GRANT.map((id) => findFeature(id)).filter((f): f is NonNullable<typeof f> =>
      Boolean(f),
    )
    ...
    console.log(
      `Ensured ${String(wanted.length)} feature rows (of ${String(FEATURES.length)} in registry).`,
    )
    ...
    console.log(
      `Done. ${String(grants)} assignments across ${String(orgs.length)} organisation(s).`,
    )
  } finally {
    await db.$disconnect()
  }
